...
{
	template< class type >
	inline i32 ListSortCompare(const type* a, const type* b) {
		return *a - *b;
	}

	template< class type >
	inline type* ListNewElement(void) {
		return new type;
	}
	...
	public:
		...
		typedef i32		cmp_t(const type*, const type*);
		typedef type	new_t(void);

		List(i32 newgranularity = 16);
		List(const List<type>& other);
		~List<type>(void);

		void			Clear(void);									// clear the list
		i32				GetNum(void) const;							// returns number of elements in list
		i32				GetAllocated(void) const;						// returns number of elements allocated for
		void			SetGranularity(i32 newgranularity);				// set new granularity
		i32				GetGranularity(void) const;						// get the current granularity
		...
		void			Resize(i32 newsize);							// resizes list to the given number of elements
		void			Resize(i32 newsize, i32 newgranularity);		// resizes list and sets new granularity
		void			SetNum(i32 newnum, bool resize = true);			// set number of elements in list and resize to exactly this number if necessary
		void			AssureSize(i32 newSize);						// assure list has given number of elements, but leave them uninitialized
		void			AssureSize(i32 newSize, const type& initValue);	// assure list has given number of elements and initialize any new elements
		void			AssureSizeAlloc(i32 newSize, new_t* allocator);	// assure the pointer list has the given number of elements and allocate any new elements

		type* GetData(void);									// returns a pointer to the list
		const type* GetData(void) const;							// returns a pointer to the list
		type& Alloc(void);									// returns reference to a new data element at the end of the list
		i32				Add(const type& obj);						// append element
		i32				Add(const List<type>& other);		// append list
		i32				AddUnique(const type& obj);						// add unique element
		i32				Insert(const type& obj, i32 index = 0);			// insert the element at the given index
		i32				FindIndex(const type& obj) const;				// find the index for the given element
		type* Find(type const& obj) const;					// find pointer to the given element
		i32				FindNull(void) const;							// find the index for the first NULL pointer in the list
		i32				IndexOf(const type* obj) const;					// returns the index for the pointer to an element in the list
		bool			RemoveIndex(i32 index);							// remove the element at the given index
		bool			Remove(const type& obj);						// remove the element


		void			Sort(cmp_t* compare = (cmp_t*)&ListSortCompare<type>);
		void			SortSubSection(i32 startIndex, i32 endIndex, cmp_t* compare = (cmp_t*)&ListSortCompare<type>);
		void			Swap(List<type>& other);				// swap the contents of the lists
	...
	/*
	================
	idList<type>::idList( i32 )
	================
	*/
	template< class type >
	inline List<type>::List(i32 newgranularity) {
	...
	*/
	template< class type >
	inline void List<type>::Clear(void) {
		if (list) {
			delete[] list;
		}
	...
	*/
	template< class type >
	inline size_t List<type>::Allocated(void) const {
	...
	*/
	template< class type >
	inline size_t List<type>::Size(void) const {
	...
	*/
	template< class type >
	inline size_t List<type>::MemoryUsed(void) const {
	...
	*/
	template< class type >
	inline i32 List<type>::GetNum(void) const {
		return num;
	}
	...
	*/
	template< class type >
	inline i32 List<type>::GetAllocated(void) const {
		return size;
	}
	...
	*/
	template< class type >
	inline void List<type>::SetNum(i32 newnum, bool resize) {
		Assert(newnum >= 0, "");
	...
	*/
	template< class type >
	inline void List<type>::SetGranularity(i32 newgranularity) {
		i32 newsize;
	...
	*/
	template< class type >
	inline i32 List<type>::GetGranularity(void) const {
		return granularity;
	}
	...
	*/
	template< class type >
	inline void List<type>::Condense(void) {
		if (list) {
			if (num) {
				Resize(num);
			}
	...
	*/
	template< class type >
	inline void List<type>::Resize(i32 newsize) {
		type* temp;
		i32		i;
	...
	*/
	template< class type >
	inline void List<type>::Resize(i32 newsize, i32 newgranularity) {
		type* temp;
		i32		i;
	...
	*/
	template< class type >
	inline void List<type>::AssureSize(i32 newSize) {
		i32 newNum = newSize;
	...
	*/
	template< class type >
	inline void List<type>::AssureSize(i32 newSize, const type& initValue) {
		i32 newNum = newSize;
	...
	*/
	template< class type >
	inline void List<type>::AssureSizeAlloc(i32 newSize, new_t* allocator) {
		i32 newNum = newSize;
	...
	*/
	template< class type >
	inline const type& List<type>::operator[](i32 index) const {
		Assert(index >= 0, "");
		Assert(index < num, "");
	...
	*/
	template< class type >
	inline type& List<type>::operator[](i32 index) {
		Assert(index >= 0, "");
		Assert(index < num, "");
	...
	*/
	template< class type >
	inline type* List<type>::GetData(void) {
		return list;
	}
	...
	*/
	template< class type >
	const inline type* List<type>::GetData(void) const {
		return list;
	}
	...
	*/
	template< class type >
	inline type& List<type>::Alloc(void) {
		if (!list) {
			Resize(granularity);
		}
	...
	*/
	template< class type >
	inline i32 List<type>::Add(type const& obj) {
		if (!list) {
			Resize(granularity);
		}
	...
	*/
	template< class type >
	inline i32 List<type>::Insert(type const& obj, i32 index) {
		if (!list) {
			Resize(granularity);
		}
	...
	*/
	template< class type >
	inline i32 List<type>::Add(const List<type>& other) {
		if (!list) {
			if (granularity == 0) {	// this is a hack to fix our memset classes
				granularity = 16;
			}
	...
	*/
	template< class type >
	inline i32 List<type>::AddUnique(type const& obj) {
		i32 index;
	...
	*/
	template< class type >
	inline i32 List<type>::FindIndex(type const& obj) const {
		i32 i;
	...
	*/
	template< class type >
	inline i32 List<type>::FindNull(void) const {
		i32 i;
	...
	*/
	template< class type >
	inline i32 List<type>::IndexOf(type const* objptr) const {
		i32 index;
	...
	*/
	template< class type >
	inline bool List<type>::RemoveIndex(i32 index) {
		i32 i;
	...
	*/
	...
	inline void List<type>::Sort(cmp_t* compare) {
		...
		typedef i32 cmp_c(const void*, const void*);
	...
	*/
	template< class type >
	inline void List<type>::SortSubSection(i32 startIndex, i32 endIndex, cmp_t* compare) {
		if (!list) {
			return;
		}
		if (startIndex < 0) {
